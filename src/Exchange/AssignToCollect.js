...
const AssignToCollect = ({ accessToken }) => {
  ...
  // Removed the problematic currentDeliveryBoyId state - not needed since we use selectedDeliveryBoy
...
const fetchOrders = async () => {
  try {
    ...
    // Filter orders with ASSIGNTOCOLLECT or RECOMPLETED status
    const requestedOrders = response.data.filter(order =>
      order.status === 'ASSIGNTOCOLLECT' || order.status === 'RECOMPLETED'
    );

    console.log("Filtered Exchange Orders:", requestedOrders);
 ...
 const handleSubmitDetails = async () => {
  ...
  // Check if delivery boy is selected
  if (!selectedDeliveryBoy) {
    Alert.alert('Error', 'Please select a delivery boy');
    return;
  }
  ...
  try {
    setSubmittingDetails(true);
      ...
      newDeliveryBoyId: selectedDeliveryBoy.userId  // Changed from newDeliveryBoyId
    };
...
>
  ...
  {deliveryBoys.map((boy, index) => (
    <TouchableOpacity
      key={`reassign-${boy.userId || boy.id}-${index}`} // Add index to ensure uniqueness
      style={[
...
// In renderDetailsModal - Replace the existing delivery boy mapping:
<ScrollView style={styles.deliveryBoyList}>
  {deliveryBoys.map((boy, index) => (
    <TouchableOpacity
      key={`details-${boy.userId || boy.id}-${index}`} // Add index to ensure uniqueness
      style={[
        styles.deliveryBoyItem,
        selectedDeliveryBoy?.userId === boy.userId && styles.selectedDeliveryBoy
      ]}
...
export default AssignToCollect; 
