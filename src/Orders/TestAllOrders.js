...
const TestAllOrders = () => {
  ...
  const filteredOrders = orders.filter(item => item.orderStatus === selectedValue && item.testUser==true);
...
<FlatList
  ...
  keyExtractor={(item, index) => item.uniqueId ? String(item.uniqueId) : String(index)}
